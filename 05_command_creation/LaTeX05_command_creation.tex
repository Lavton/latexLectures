...
\subtitle{Command creation}
\author{Anton Lioznov}
\institute{Skoltech}
\date{ISP 2019,\\ \textit{lesson 5}}
\frame{\titlepage}
\begin{frame}[allowframebreaks]\frametitle{What we will know?}
...
\AtBeginSection[] 
{
  \begin{frame}[allowframebreaks]{What we will know?}
...
\section{Simple command creation}
\input{sec02/sec02}

\section{Simple .sty and .cls file creation}
\input{sec03/sec03}
...
\section{\LaTeX's advanced creation}
\input{sec07/sec07}

\section{Relatate stuff}
\input{sec08/sec08}
%write/read, expand,...
...
\begin{frame}[allowframebreaks]\frametitle{What we have learned today?}
\tableofcontents
\end{frame}
...
color from the footnotes corresponds to references' color.
    \begin{itemize}
        ...
        \item \url{http://ctan.altspu.ru/info/macros2e/macros2e.pdf} list of internal \LaTeX\ macros
        \item \url{https://www.latex-project.org/help/documentation/clsguide.pdf} guide for class and package writers
        % \item \tikzc{TikZ manual} (1161 pages :))
    \end{itemize}
\end{frame}
