...
\begin{frame}[fragile]{Cons}\relax
     \begin{itemize}
        ...
        \item[$-$] when you want to do something {\csk ``against the rules''} %\vspace{-1ex}
        \item[$-$] when you want to work with {\csk visual-based} things (tables, pictures) % \vspace{-1ex}
        \item[$-$] when you want to do something really {\csk simple} %\vspace{-1ex}
        \item[$-$] when you want to do something {\csk ``quick and dirty''} %\vspace{-1ex}
    \end{itemize}
    
    \cprotect\skfootnote{
here \verb|\item[$-$]| was used
}
\end{frame}
...
\begin{frame}{Pros}\relax
     \begin{itemize}
        \item[$+$] When you have lots of {\csk equations} %\vspace{-1ex}
        \item[$+$] When you have a {\csk complex, but typical} document %\vspace{-1ex}
        \item[$+$] When you care about {\csk device-independent} view and edit %\vspace{-1ex}
        \item[$+$] When you don't want {\csk not care about the beauty}, but want it%\vspace{-1ex}
        \item[$+$] When you are {\csk care about the beauty} very much%\vspace{-1ex}
        \inclass{\item[$+$]\inclasshigh{} When you hate all these {\csk humanitarian} GUI programs}
        \item[$+$] When you {\csk love text files} 
        \inclass{\item[$+$]\inclasshigh{} When you want to {\csk impress} your partner}
    \end{itemize}
\end{frame}
...
\begin{frame}[fragile]{Common belief}\relax
    \begin{center}
        \begin{tikzpicture}
             \node[align=center] (0,0) {\huge \LaTeX\ is only for use\\ \huge  in academic area};
...
\begin{frame}[fragile]{Illustration when to use \LaTeX}\relax
\begin{center}
